...
#define S650_FIRMWARE   "dvb-usb-s650.fw"
...
/* TODO */
static struct dvb_frontend *tda18273_attach(struct dvb_frontend *fe,
						   struct i2c_adapter *i2c,
						   const u8 i2c_addr)
{
	printk(KERN_WARNING "%s: driver not support...TODO...\n", __func__);
	return NULL;
}
...
static int prof_rc_query(struct dvb_usb_device *d)
{
	...
	if (d->props.i2c_algo->master_xfer(&d->i2c_adap, &msg, 1) == 1) {
		if (msg.buf[0] != 0xff) {
			deb_rc("%s: rc code: %x, %x\n",
					__func__, key[0], key[1]);
			rc_keydown(d->rc_dev, RC_TYPE_UNKNOWN, key[0] ^ 0xff,
				   0);
		}
	}
...
static struct dvb_usb_device_properties *s650;
static const struct dvb_usb_device_description d650 = {
	"TeVii S650 USB2.0",
		...
		const struct usb_device_id *id)
{
	...
	s650 = kmemdup(&dw2104_properties,
		       sizeof(struct dvb_usb_device_properties), GFP_KERNEL);
	if (!s650)
		goto err3;

	s650->firmware = S650_FIRMWARE;
	s650->devices[0] = d650;
	s650->rc.core.rc_codes = RC_MAP_TEVII_NEC;
	...
	if (0 == dvb_usb_device_init(intf, &dw2102_properties,
	    ...
	    0 == dvb_usb_device_init(intf, &dw2104_properties,
			THIS_MODULE, NULL, adapter_nr) ||
	    0 == dvb_usb_device_init(intf, s650,
			THIS_MODULE, NULL, adapter_nr) ||
