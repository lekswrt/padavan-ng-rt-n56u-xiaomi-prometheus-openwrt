		...
		int *state)
{
	...
	*state = REMOTE_NO_KEY_PRESSED;
	...
	if (cxusb_ctrl_msg(d, 0x10, NULL, 0, ircode, 2) < 0)
		return 0;
/*
	for (i = 0; i < d->props.rc.legacy.rc_map_size - 1; i++) {
...
static int cxusb_t230_rc_query(struct dvb_usb_device *d, u32 *event,
		int *state)
{
	struct rc_map_table *keymap = d->props.rc.legacy.rc_map_table;
	u8 ircode[2];
	...
	*state = REMOTE_NO_KEY_PRESSED;
	...
	if (cxusb_ctrl_msg(d, 0x10, NULL, 0, ircode, 2) < 0)
		return 0;
	if (!(ircode[0] == (u8)0xff && ircode[1] == (u8)0xff)) {
		printk(KERN_INFO "%s: rc_map_size == %i\n", __func__, d->props.rc.legacy.rc_map_size);
		printk(KERN_INFO "%s: ircode[0] == %u, ircode[1] == %u\n", __func__, (unsigned)ircode[0], (unsigned)ircode[1]);
	for (i = 0; i < (unsigned)d->props.rc.legacy.rc_map_size; i++) {
/*
		if (rc5_custom(&keymap[i]) == ircode[0] &&
		    rc5_data(&keymap[i]) == ircode[1]) {
			*event = keymap[i].keycode;
